...
## How to Physically Move the Files

### In VS Code

- Right click the project folder
- Create the new folders
- Drag the files into them

### Or in terminal

```bash
mkdir -p models utils scripts checkpoints logs config

mv model.py models/
mv tiny_llm_model.py models/
mv tiny_vlm_model.py models/

mv llm_utils.py utils/
mv vlm_utils.py utils/

mv train.py scripts/
mv train_tiny_llm.py scripts/
mv train_tiny_vlm.py scripts/
mv verification.py scripts/

mv mnist_cnn.pth checkpoints/
mv mnist_dnn.pth checkpoints/
mv mnist_tiny_llm.pth checkpoints/
mv mnist_tiny_vlm.pth checkpoints/

mv emissions.csv logs/
mv fingerprint_master_data.csv logs/
mv powermetrics_log.txt logs/

mv device_id.txt config/

touch models/__init__.py utils/__init__.py scripts/__init__.py config/__init__.py
```
